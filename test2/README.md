...
<br>三个角色共同拥有登录用例，读者和图书管理员还有注册账号和修改个人信息功能，而系统管理员拥有审核其他两种角色注册账号的用例。其中修改个人信息和继承于修改信息用例。图书管理员拥有录入图书和删除图书以及修改图书信息用例，其中修改图书信息用例也继承于修改信息用例，删除和例如图书用例包含了登录用例。读者还拥有借阅图书和归还图书以及缴纳罚金的用例，其中缴纳罚金包含了登录用例。
<br>include关系表示包含，一个__主要__功能__必须__在某个功能基础之上进行，在界面或者接口中表现为嵌套形式，比如借书必须先查询，而extend则是__次要__的功能，延伸出来的，比如续借是借书的衍生物，可选的。
